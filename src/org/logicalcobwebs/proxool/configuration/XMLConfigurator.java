 ...
 * {@link JAXPConfigurator} will do this for you.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * See the <a href="The latest version is available at
 * http://proxool.sourceforge.net/properties.html" target="_new">Proxool
 * properties</a> for documentation on the available configuration properties.
 * </p>
 *
 ...
 * <ul>
 * <li>Put your proxool configuration elements inside a root
 * <code>proxool-config</code> element. The document must adhere to the <a
 * href="proxool.dtd">Proxool dtd</a>.</li>
 * <li>Add a <code>DOCTYPE</code> entry to your xml with a system id containing
 * the <i>absolute url</i> to the Proxool dtd. The Proxool jar contains a copy
 * of the Proxool dtd in the confguration package. You can reference that with a
 ...
 * this is done by passing <code>true</code> as the second arghument to any of
 * the <code>configure</code> methods.</li>
 * </ul>
 * </p>
 * <p>
...
public class XMLConfigurator extends DefaultHandler {
	...
	public void startElement(String uri, String lname, String qname,
		...
		if (elementName.equals(PROXOOL)) {
			if (insideProxool) {
				throw new SAXException("A <" + PROXOOL
						+ "> element can't contain another <" + PROXOOL
						+ "> element.");
			}
			insideProxool = true;
			properties.clear();
		...
		if (elementName.equals(PROXOOL)) {
			...
			if (driverClass == null || driverUrl == null) {
				throw new SAXException("You must define the "
						+ ProxoolConstants.DRIVER_CLASS + " and the "
						+ ProxoolConstants.DRIVER_URL + ".");
			}
			...
			// Register the pool
			try {
				final String ec = properties
						.getProperty(ProxoolConstants.PROPERTY_PREFIX
				...
				if (ec != null) {// 判断配置文件中是否有解密class的配置，如果有则动态加载
					...
					final String user = properties
							.getProperty(ProxoolConstants.USER_PROPERTY);// 取原值
					properties.setProperty(ProxoolConstants.USER_PROPERTY,
							dec.decrypt(user));// 解密并设置
					final String password = properties
							.getProperty(ProxoolConstants.PASSWORD_PROPERTY);// 取原值
					properties.setProperty(ProxoolConstants.PASSWORD_PROPERTY,
							dec.decrypt(password));// 解密并设置
				}
				ProxoolFacade
						.registerConnectionPool(url.toString(), properties);
		...
		if (localName.equals(ProxoolConstants.ALIAS)) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Setting property '"
						+ ProxoolConstants.PROPERTY_PREFIX + localName
						+ "' to value '" + value + "'.");
			}
	...
	private void setDriverProperty(Attributes attributes) throws SAXException {
		...
		if (name == null || name.length() < 1 || value == null) {
			throw new SAXException(
					"Name or value attribute missing from property element."
	...
	private boolean namespaceOk(String uri) {
		return uri == null || uri.length() == 0
				|| uri.equals(ProxoolConstants.PROXOOL_XML_NAMESPACE_URI);
	}
}
